...
export default function Demo() {
  ...
  return (
    <section id="demo" className="bg-[#030712] py-20 text-white border-t border-gray-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Demo Interface Card */}
        <div className="group bg-[#080c14] border border-gray-900/80 hover:border-gray-800 rounded-xl grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-900/80 text-left overflow-hidden hover:shadow-[0_0_40px_rgba(59,130,246,0.07)] transition-all duration-500">
          {/* Input Box Column */}
          <div className="p-6 flex flex-col justify-between min-h-[300px]">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-white">
                  ...
                  <span className="text-[10px] text-gray-500 font-medium">5-star review</span>
                </div>
                <span className="text-[9px] text-gray-600 tracking-wider uppercase font-semibold">Input</span>
              </div>
              <textarea
                className="w-full bg-transparent text-xs text-gray-400 focus:text-white resize-none outline-none focus:ring-0 leading-relaxed h-32 transition-colors duration-300"
                value={reviewInput}
            ...
            {/* Controls */}
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-gray-900/40">
              <div className="flex gap-1 items-center bg-[#030712] border border-gray-900 p-0.5 rounded-lg text-[10px]">
                {['Warm', 'Professional', 'Concise'].map((t) => (
                  <button
                    key={t}
                    onClick={() => setTone(t)}
                    className={`px-2.5 py-1 rounded-md font-medium transition-all duration-300 ${tone === t ? 'bg-white text-black shadow-sm scale-105' : 'text-gray-500 hover:text-gray-300 hover:bg-[#111827]'}`}
                  >
                    {t}
                  </button>
                ))}
              ...
              <button
                ...
                className="bg-white text-black text-[11px] font-bold px-3 py-1.5 rounded-lg hover:bg-gray-200 hover:scale-105 active:scale-95 transition-all duration-300 shadow-sm"
              >
          ...
          <div className="p-6 flex flex-col justify-between bg-[#050910] group-hover:bg-[#060a12] transition-colors duration-500 min-h-[300px]">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] text-gray-600 tracking-wider uppercase font-semibold">Output</span>
              </div>
              <p className={`text-xs leading-relaxed transition-all duration-500 ${isGenerating ? 'opacity-40 animate-pulse blur-[1px]' : 'text-gray-300 blur-0'}`}>
                {aiOutput}
              </p>
            </div>
